function Watch-VSStackEvents {
    ...
    if ($InNewWindow) {
        if ($Env:OS -and ($Env:OS -notlike "*Windows*")) {
            ...
            $command = "Import-Module `"$($Script:VaporshellPath)\Vaporshell.psd1`" -ArgumentList `$false,`$true -Verbose:`$false;Clear-Host;. `"$($Script:VaporshellPath)\bin\SetWatchWindow.ps1`";Watch-VSStackEvents -StackId '$StackId' -RefreshRate $RefreshRate"
            if ($ProfileName) {
